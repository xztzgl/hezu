...
public interface OrderListRepository extends JpaRepository<OrderProductView,Integer> {
    ...
    List<OrderProductView> findByCustomer_id(@Param("customer_id") String customer_id);

    @Query(value = "select a From OrderProductView a where a.customer_id = :customer_id and a.status_id = :status_id ")
    Page<OrderProductView> findByCustomer_idAndStatus_id(@Param("customer_id") String customer_id, @Param("status_id") String status_id, Pageable pageRequest);
